...
import { Star, MapPin, Video, Home, ShieldCheck, MessageCircle } from "lucide-react";
...
export const PhysioCard = ({ physio }: { physio: Physio }) => (
  <Card className="overflow-hidden shadow-card hover:shadow-elevated transition-smooth group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            {physio.verified && (
              <div className="mt-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-health text-health-foreground text-[11px] font-bold uppercase tracking-wide">
                <ShieldCheck className="h-3 w-3" strokeWidth={3} /> Verificado
              </div>
            )}
            <div className="text-[11px] text-health font-medium mt-1 truncate">
    ...
    <div className="px-5 pb-5 flex gap-2">
      ...
      <Button variant="outline" size="icon" asChild aria-label="Enviar mensaje">
        <Link to="/dashboard/mensajes"><MessageCircle className="h-4 w-4" /></Link>
      </Button>
    </div>
  </Card>
);
